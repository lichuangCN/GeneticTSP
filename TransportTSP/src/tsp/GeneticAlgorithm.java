...
public class GeneticAlgorithm {
	 ...
	 * 个体染色体合法性调整
	 * @param population
	 * @param noArriveCities
	 */
	public void judgement(Population population, int[][] noArriveCities) {
		for (int populationIndex = 0; populationIndex < populationSize; populationIndex++) {
			Traveler traveler = population.getTraveler(populationIndex);
			// 检测到非法，调整, 直到合法为止
			while (traveler.isLegalRoute(traveler.getChromosome(), noArriveCities) != 0) {				
				traveler.changePostions(traveler.getChromosome(), 
						traveler.isLegalRoute(traveler.getChromosome(), noArriveCities));
			}
		}
	}
}
